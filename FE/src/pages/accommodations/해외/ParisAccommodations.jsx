// src/pages/accommodations/ParisAccommodations.jsx
...
// ✅ 로컬 폴백 이미지
import IMG_PARIS_HOTEL from "../../../assets/images/overseas/파리 호텔.jpg";
...
const onlyParis = (a) => {
  ...
  const keywords = [
    ...
    "루브르", "louvre",
    "오르세", "orsay",
    "샹젤리제", "champs-elysees", "champs élysées", "champs elysees",
    "라데팡스", "la defense", "la défense",
    "몽마르뜨", "montmartre",
    "오페라", "opera",
    "마레", "le marais", "marais",
    "생제르맹", "saint-germain", "saint germain",
    "라탱", "latin quarter",
  ];
...
const LOCAL_FALLBACKS = {
  [toSlug("파리 호텔")]: IMG_PARIS_HOTEL,
};

// ✅ 썸네일 포커스(없으면 중앙)
const FOCUS_MAP = {
  [toSlug("파리 호텔")]: { x: "50%", y: "50%" },
};

/* ====================== 컴포넌트 ====================== */
export default function ParisAccommodations() {
  ...
  // ===== 데이터 로드 (파리 + 해외 rooms 최저가 병합) =====
  const fetchParis = useCallback(async () => {
    setLoading(true);
    setErrMsg("");
    setPage(1);
    ...
    try {
      ...
      const res = await AxiosClient("accommodations").get("", {
        params: { region: "파리", location: "파리", keyword: "파리" },
      });
      ...
      console.error(e);
      setErrMsg("파리 숙소를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setItems([]);
    } finally {
      setLoading(false);
    }
