// src/pages/accommodations/FukuokaAccommodations.jsx
import React, {
  useEffect,
  useMemo,
  useState,
...
// '후쿠오카' 포함 여부 (오타/영문/일문까지 허용)
const onlyFukuoka = (a) => {
  const text = `${a?.name ?? ""} ${a?.location ?? ""} ${a?.address ?? ""}`.toLowerCase();
  // 한글(후쿠오카/호쿠오카), 영문, 일본어(福岡) 모두 체크
  return (
    text.includes("후쿠오카") ||
    text.includes("호쿠오카") ||
    text.includes("fukuoka") ||
    `${a?.name ?? ""}${a?.location ?? ""}${a?.address ?? ""}`.includes("福岡")
  );
};
...
export default function FukuokaAccommodations() {
  ...
  const [items, setItems] = useState([]); // 후쿠오카만
  ...
  // ===== 데이터 로드 (후쿠오카만) =====
  const fetchFukuoka = useCallback(async () => {
    setLoading(true);
    setErrMsg("");
    setPage(1);

    try {
      // 1) 서버에서 지역 파라미터로 우선 시도 (한글 기준)
      const res = await AxiosClient("accommodations").get("", {
        params: { region: "후쿠오카", location: "후쿠오카", keyword: "후쿠오카" },
      });
      ...
      console.error(e);
      setErrMsg("후쿠오카 숙소를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setItems([]);
    } finally {
      setLoading(false);
    }
