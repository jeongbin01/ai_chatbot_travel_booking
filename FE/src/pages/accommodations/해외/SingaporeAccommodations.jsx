...
// '싱가포르' 포함 여부 (ko/en/zh/ms 대응)
const onlySingapore = (a) => {
  const lower = `${a?.name ?? ""} ${a?.location ?? ""} ${a?.address ?? ""}`.toLowerCase();
  const raw = `${a?.name ?? ""}${a?.location ?? ""}${a?.address ?? ""}`;
  return (
    lower.includes("싱가포르") ||      // Korean
    lower.includes("singapore") ||     // English
    raw.includes("新加坡") ||          // Chinese
    lower.includes("singapura") ||     // Malay
    lower.includes("sg")               // 흔한 약칭(오탐 가능하지만 해외 데이터에 유용)
  );
};
...
export default function SingaporeAccommodations() {
  ...
  const [items, setItems] = useState([]); // 싱가포르만
