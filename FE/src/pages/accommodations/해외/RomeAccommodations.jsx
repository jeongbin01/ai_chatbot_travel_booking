...
// '로마' 포함 여부 (ko/en/it 대응)
const onlyRome = (a) => {
  const lower = `${a?.name ?? ""} ${a?.location ?? ""} ${a?.address ?? ""}`.toLowerCase();
  const raw = `${a?.name ?? ""}${a?.location ?? ""}${a?.address ?? ""}`;
  return (
    lower.includes("로마") ||     // Korean
    lower.includes("rome") ||    // English
    lower.includes("roma") ||    // Italian
    raw.includes("Lazio") ||     // 지역 표기만 있는 경우
    raw.includes("Città metropolitana di Roma") // 공식 표기
  );
};
...
export default function RomeAccommodations() {
  ...
  const [items, setItems] = useState([]); // 로마만
  ...
  // ===== 데이터 로드 (로마만) =====
  const fetchRome = useCallback(async () => {
    setLoading(true);
    setErrMsg("");
    setPage(1);
    ...
    try {
      ...
      const res = await AxiosClient("accommodations").get("", {
        params: { region: "로마", location: "로마", keyword: "로마" },
      });
      ...
      console.error(e);
      setErrMsg("로마 숙소를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setItems([]);
    } finally {
      setLoading(false);
    }
