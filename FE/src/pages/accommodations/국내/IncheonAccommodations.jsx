// src/pages/accommodations/IncheonAccommodations.jsx
...
// '인천' 영역만 필터(송도/강화/영종 등 서브지역 포함)
const onlyIncheon = (a) => {
  const text = `${a?.name ?? ""} ${a?.location ?? ""} ${a?.address ?? ""}`.toLowerCase();
  const keywords = ["인천", "송도", "강화", "강화도", "영종", "을왕리", "왕산", "청라"];
...
// ✅ 로컬 폴백 매핑(데이터 name과 일치해야 적용)
const LOCAL_FALLBACKS = {
  [toSlug("송도 호텔")]: IMG_SONGDO_HOTEL,
  [toSlug("강화도 펜션")]: IMG_GANGHWADO_PENSION,
};

// ✅ 썸네일 포커스(없으면 중앙)
const FOCUS_MAP = {
  [toSlug("송도 호텔")]: { x: "50%", y: "45%" },
  [toSlug("강화도 펜션")]: { x: "50%", y: "55%" },
};

/* ====================== 컴포넌트 ====================== */
export default function IncheonAccommodations() {
  ...
  // ===== 데이터 로드 (인천 + 최저가 병합) =====
  const fetchIncheon = useCallback(async () => {
    setLoading(true);
    setErrMsg("");
    setPage(1);
    try {
      ...
      const res = await AxiosClient("accommodations").get("", {
        params: { region: "인천", location: "인천", keyword: "인천" },
      });
      ...
      console.error(e);
      setErrMsg("인천 숙소를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setItems([]);
    } finally {
      setLoading(false);
    }
