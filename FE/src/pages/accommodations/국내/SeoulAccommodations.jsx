...
export default function SeoulAccommodations() {
  ...
  const [items, setItems] = useState([]); // 서울만
  ...
  // ===== 데이터 로드 (서울만) =====
  const fetchSeoul = useCallback(async () => {
    setLoading(true);
    setErrMsg("");
    setPage(1);
    ...
    try {
      ...
      const res = await AxiosClient("accommodations").get("", {
        params: { region: "서울", location: "서울", keyword: "서울" },
      });
      ...
      console.error(e);
      setErrMsg("서울 숙소를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  }, [priceRange.lo, priceRange.hi]); // eslint-disable-line react-hooks/exhaustive-deps
