...
const accommodationsMock = [
  {
    ...
    price: 100000,
    averageRating: 4.5,
    image: "/images/default-accommodation.jpg",
  },
  {
    id: 2,
    name: "테스트 숙소 2",
...
const BookingPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 2) Mock 폴백
        const fallback = accommodationsMock.find(
          (item) => item.id === parseInt(id, 10)
        );
        if (fallback) {
          if (!ignore) setAccommodation(fallback);
        } else {
          if (!ignore)
            setError(
  ...
  const handleBooking = () => {
    ...
    const newBooking = {
      ...
      nights,
      totalPrice,
      paymentMethod: location.state?.paymentMethod || "미입력",
